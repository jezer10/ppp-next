...
const studentOptionItems = [
  { icon: IdentificationIcon, name: 'Info Estudiante' },
  { icon: ArrowTrendingUpIcon, name: 'Estado' },
  { icon: DocumentIcon, name: 'Documentos' },
  { icon: UserGroupIcon, name: 'Cambiar Supervisor' },
  { icon: TrashIcon, name: 'Eliminar' },
];
const studentList = [
  { id: 1, show: false },
  { id: 1, show: false },
    ...
    <div className="h-full">
      ...
      <div className="overflow-x-auto ">
        <table className=" w-full border-separate border-spacing-x-0 border-spacing-y-4">
          <thead className=" border-[#EEE]">
            <tr className="bg-white  text-left text-[0.6875rem] text-[#757575] font-medium">
              <th className="rounded-l-lg pl-4 py-3">Código</th>
              ...
              <th>Supervisor</th>
              <th>Tiempo</th>
              <th className="rounded-r-lg">Opciones</th>
            </tr>
          </thead>
          <tbody>
            {studentList.map((e, idx) => (
              <>
                <tr
                  key={idx}
                  className="bg-white  text-left text-[0.625rem]  text-[#C4C4C4] font-normal"
                >
                  <td className="rounded-l-lg pl-4 py-6">Código</td>
                ...
                {e?.show && (
                  <tr>
                    <td colSpan={8}>
                      <div className="flex p-4 bg-[#D1D1D1] items-center rounded-b-lg">
                        <div className="rounded-[0.625rem] px-4 py-3 text-white bg-[#55E38E] flex items-center gap-2">
    ...
    <Menu as="div" className="relative inline-block text-left">
      ...
      >
        <Menu.Items className="absolute right-0 mt-2 z-10 w-32 overflow-hidden origin-top-right rounded-lg bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
          {studentOptionItems.map(({ name, icon: Icon }, ind) => (
            <Menu.Item key={ind}>
              {({ active }) => (
                <button
                  className={`${
                    active && 'bg-[#EEEEEE]'
                  } flex w-full items-center gap-2 text-[0.5rem] py-1 px-2 whitespace-nowrap`}
                >
