...
function EvaluationComponent() {
  ...
  const [isOpenSuccessManage2, setIsOpenSuccessManage2] = useState({
    state: false,
    message: "",
  }); // change to isOpenSuccessManage
  ...
  const deleteAssesment = async (id: number | null) => {
    try {
      ...
      setIsOpenSuccessManage3({
        state: true,
        message: "Se ha eliminado la evaluación correctamente.",
      });
      fetchProcesses();
    } catch (error) {
  ...
  function AssesmentOptions(props: any) {
    const processFound = processList.find(
      (x) => x.process_id === props.processId,
    );
    const assesmentFound = processFound?.Evaluacion.find(
      (y) => y.assesment_id === props.assesmentId,
    );
              ...
              ({ name, actionFunction, icon: Icon }, idx) => (
                <Menu.Item key={idx}>
                  {({ active }) => (
                    <button
                      onClick={() =>
                        actionFunction(props.processId, props.assesmentId)
                      }
                      className={`${
                        active && "bg-[#EEEEEE]"
                      } flex w-full items-center gap-2 whitespace-nowrap px-2 py-1 text-[0.5rem] text-[#757575]`}
  ...
  const handleSuccessModalManage2 = () => {
    setIsOpenSuccessManage2({
      state: !isOpenSuccessManage2.state,
      message: "",
    });
  };
  ...
  const [isOpenSuccessManage3, setIsOpenSuccessManage3] = useState({
    state: false,
    message: "",
  }); // change to isOpenSuccessManage
  ...
  const handleSuccessModalManage3 = () => {
    setIsOpenSuccessManage3({
      state: !isOpenSuccessManage3.state,
      message: "",
    });
  };
  ...
  if (performAssessment) {
    return (
      <PerformAssessment
        onBack={handleBackToMain2}
        manageSuccess={manageSuccess2}
        assesmentId={selectedId}
      />
    );
  }
  ...
  return (
    <>
      ...
      <Modal isOpen={modalAssesment} onClose={handleModalAssesment}>
        {messageAssesment}
      </Modal>
                  ...
                  >
                    ...
                    {item.show && (
                      <div className="flex items-center gap-4  rounded-b-lg p-4">
                        ...
                        {item.Evaluacion.map(
                          (assesment: IAssesment, documentIndex) => (
                            <div
                              key={documentIndex}
                              ...
                              <div className="flex flex-col gap-2">
                                <div className="flex flex-row items-center justify-between gap-2">
                                  <div className="flex flex-row items-center gap-2">
                                    ...
                                    <div>
                                      <div className="whitespace-nowrap text-[0.625rem] font-bold">
                                        {Number(assesment.type) === 0
                                          ? "Ev. Inicial"
                                          : Number(assesment.type) === 1
                                            ? "Ev. Intermedia"
                                            : "Ev. Final"}
                                      </div>
                                  ...
                                  <AssesmentOptions
                                    ...
                                    assesmentId={assesment.assesment_id}
                                  />
                                </div>
                                ...
                                {assesment.status === 1 ? (
                                  <span className="text-xs">
                                    ...
                                    convertToLocalDate(
                                      convertToStringDate(assesment.start_date),
                                    ) &&
                                  _currentDate <
                                    convertToLocalDate(
                                      convertToStringDate(assesment.end_date),
                                    ) ? (
                                  <span className="text-xs">
                                    {" "}
