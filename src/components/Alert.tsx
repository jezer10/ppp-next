    ...
    <>
      {isOpen && (
        <div className="fixed bottom-0 left-0 right-0 top-0 z-[999] flex items-center justify-center">
          ...
          <div className="absolute  inset-0 z-[51] m-auto h-fit rounded-lg bg-white p-6 md:w-[90%] lg:w-[40%]">
            {!isLoading && (
              <>
                ...
                <div className="flex w-full flex-col items-center gap-5">
                  ...
                  <div className="text-md text-center">{message}</div>
                  <div className="mt-4 flex justify-center">
                    <button
                      onClick={onCancel}
                      className="mr-2 flex min-h-full items-center gap-3 rounded-lg bg-red-500 p-3 text-white"
                    >
                      Cancelar
                    </button>
                    <button
                      onClick={onConfirm}
                      className="mr-2 flex min-h-full items-center gap-3 rounded-lg bg-green-500 p-3 text-white"
                    >
                      Continuar
                    </button>
                  </div>
            ...
            {isLoading && (
              <div className="flex w-full flex-col items-center gap-5 py-14">
                <div role="status">
                  <svg
                    aria-hidden="true"
                    className="mr-2 h-10 w-10 animate-spin fill-[#FF9853] text-gray-200 dark:text-gray-600"
                    viewBox="0 0 100 101"
                    fill="none"
    ...
    <>
      {isOpen && (
        <div className="fixed bottom-0 left-0 right-0 top-0 z-[999] flex items-center justify-center">
          <div
            onClick={onClose}
            className="absolute h-full w-full bg-[#313131]/80"
          ></div>
          <div className="absolute  inset-0 z-[51] m-auto h-fit rounded-lg bg-white p-6 md:w-[90%] lg:w-[40%]">
            <div className="flex w-full justify-end">
              <button
      ...
      {isOpen && (
        <div className="fixed bottom-0 left-0 right-0 top-0 z-[999] flex items-center justify-center">
          <div
            ...
            className="absolute h-full w-full bg-[#313131]/80"
          ></div>
          <div className="absolute z-[51] rounded-lg bg-white p-6 md:w-[90%] lg:w-[40%]">
            <div className="flex w-full justify-end">
              <button
                onClick={onClose}
                className="rounded-lg p-1 hover:bg-slate-100"
